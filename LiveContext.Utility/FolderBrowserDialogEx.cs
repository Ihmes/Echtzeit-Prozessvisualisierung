...
{
    public class FolderValidationEventArgs : EventArgs
    {
        public string path;
        public bool isValid;
    }
    ...
    public class FolderBrowserDialogEx : System.Windows.Forms.CommonDialog
    {
        ...
        private readonly ResourceManager localization = new ResourceManager("LiveContext.Utility.Localization.LiveContextUtility",
        ...
        private bool _validateEditText;
        ...
        private String _manuallyEnteredPath = "";

        public event EventHandler<FolderValidationEventArgs> ValidateFolder;
        ...
        public FolderBrowserDialogEx()
        {
            ...
            this._validateEditText = false;
        }

        public FolderBrowserDialogEx(bool validateEditText)
        {
            this.Reset();
            this._showEditBox = true;
            this._validateEditText = validateEditText;
        }
        // Factory Methods
        ...
        public void SetWindowText(string _text)
        {
            PInvoke.User32.SetWindowText(_hwndEdit, _text);
        }
        ...
        private int FolderBrowserCallback(IntPtr hwnd, int msg, IntPtr lParam, IntPtr lpData)
        {
            ...
            switch (msg)
            {
                case BrowseForFolderMessages.BFFM_INITIALIZED:
                    ...
                    {
                        // validation activated, disable ok button per default
                        // Commented for bugfix 6170
                        //PInvoke.User32.SendMessage(new HandleRef(null, hwnd), BrowseForFolderMessages.BFFM_ENABLEOK, 0, 0);
                    }
                    if (this._showEditBox && !this._validateEditText)
                    {
                        _hwndEdit = PInvoke.User32.FindWindowEx(new HandleRef(null, hwnd), IntPtr.Zero, "Edit", null);
                        PInvoke.User32.EnableWindow(_hwndEdit, false); 
                    }
                    break;
                ...
                case BrowseForFolderMessages.BFFM_SELCHANGED:
                    ...
                    if (pidl != IntPtr.Zero)
                    {
                        ...
                        else
                        {
                            ...
                            /* livecontext folder check */
                            var e = new FolderValidationEventArgs { isValid = true, path = displayedPath };
                            if (ValidateFolder != null) ValidateFolder(this, e);
                            haveValidPath = e.isValid;
                            ...
                            _lastSelectedPath = displayedPath;
                ...
                case BrowseForFolderMessages.BFFM_VALIDATEFAILEDW:
                        bool bLastSelectedPath = false;
                        if (!string.IsNullOrEmpty(_lastSelectedPath))
                        {
                            bLastSelectedPath = true;
                        }


                        string newPath = "";
                        bool bValidPath = false; 
                        if (bLastSelectedPath == true)
                        {
                            string edtText = Marshal.PtrToStringAuto((IntPtr)lParam);

                            newPath = _lastSelectedPath + "\\" + edtText;
                            if (Directory.Exists(newPath))
                            {
                                var ev = new FolderValidationEventArgs { isValid = true, path = newPath };
                                if (ValidateFolder != null) ValidateFolder(this, ev);
                                bValidPath = ev.isValid;
                            }
                            else
                            {
                                try
                        ...
                        if (!bValidPath)
                        {
                            string mess = localization.GetString("Message.SelectDirectory.Error");
                            mess = string.Format(mess, newPath);
        ...
        protected override bool RunDialog(IntPtr hWndOwner)
        {
            ...
            if (this._validateEditText)
                _uiFlags += BrowseFlags.BIF_VALIDATE;
            ...
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(_manuallyEnteredPath))
                    {
                        this._selectedPathNeedsCheck = true;
                        this._selectedPath = _manuallyEnteredPath;
                        result = true;
                    }
